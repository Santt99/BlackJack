##	Title: BlackJack
##	Des: It is a blackjack game made in python
##	Authors: Santiago Juarez, Aidan Yepson
##	Date: February 20 of 2019
	...
	while player.get_turn():
		...
		if(int(player.get_score())==21):
			...
			if (int(player.get_score())>21):
				...
				print("Type (yes) to get another card, else time anything")
				res=input()
				if (res=="yes"):
					cards=player.get_cards(cards)
					player.set_score()
...
def play_again():
	...
	global plays
	print("Player money: "+str(player.get_money()))
	print("Type (yes) to play again, else type anything")
	text=input()
	if (text=="yes"):
		play=True
		reset_game()
